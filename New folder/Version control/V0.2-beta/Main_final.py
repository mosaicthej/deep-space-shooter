...
def main():
    ...
    Bob = classes_final.Player(GREEN, x, y)

    bobo = classes_final.Player(GREEN,x,y)
    ...
    #Fred = classes_final.Enemy(Fred_test_image,RED,0,-300,178,269,1000,2.5,0,100)
    ...
    def create_enemy():
        ...
        for i in range (10):
            ...
            color = (math.floor((health_variable-20)*5/8),math.floor(250-(health_variable-20)*15/8),math.floor((health_variable-20)*5/8))
            goblin = classes_final.Bird(0,color,random.randint(0,100),random.randint(-300,0),health_variable,health_variable,health_amount,5-(health_variable-21)/16,0,length_health,1,30)
    ...
    Collins = classes_final.Enemy(0,WHITE,-10,-60,570,50,10**2000,0,0,0,0,10)
    ...
    ##    bullet_player = classes_final.Bullet(ocu.randrgb(),(Bob.rect.x + width_player/2),bobo.rect.y,0,-10,[2,2])
    ##
    ##    if not immune:
    ...
    ##        all_sprites_list.add(bullet_player)
        for i in range (1,ply_power):
            if not immune:
                ply_bullets.add(classes_final.Bullet(ocu.randrgb(),(Bob.rect.x+3 + width_player/2),bobo.rect.y,random.randint(-1*i+1,i-1),-15,[2,2]))
                all_sprites_list.add(classes_final.Bullet(ocu.randrgb(),(Bob.rect.x+3 + width_player/2),bobo.rect.y,random.randint(-1*i+1,i-1),-15,[2,2]))
