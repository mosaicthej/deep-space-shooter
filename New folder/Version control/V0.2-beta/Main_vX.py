...
Bob = classes_vX.Player(GREEN, x, y)

bobo = classes_vX.Player(GREEN,x,y)
...
#Fred = classes_vX.Enemy(Fred_test_image,RED,0,-300,178,269,1000,2.5,0,100)
#all_sprites_list.add(Fred)
#enemy_list.append(Fred)
def create_enemy():
    for i in range (10):
        wave_modifier = 1 + (wave/10)
        health_variable = random.randint(20,100)
        health_amount = health_variable*wave_modifier
        length_health = health_variable/2
        goblin = classes_vX.Bird(0,((health_variable-20)*5/8,250-(health_variable-20)*15/8,(health_variable-20)*5/8),random.randint(0,100),random.randint(-300,0),health_variable,health_variable,health_amount,5-(health_variable-19)/16,0,length_health,1,30)
...
Collins = classes_vX.Enemy(0,WHITE,-10,-60,570,50,10**2000,0,0,0,0,10)
...
##    bullet_player = classes_vX.Bullet(ocu.randrgb(),(Bob.rect.x + width_player/2),bobo.rect.y,0,-10,[2,2])
##
##    if not immune:
...
##        all_sprites_list.add(bullet_player)
    ...
    for i in range (1,ply_power):
        if not immune:
            ply_bullets.add(classes_vX.Bullet(ocu.randrgb(),(Bob.rect.x+3 + width_player/2),bobo.rect.y,random.randint(-1*i+1,i-1),-15,[2,2]))
            all_sprites_list.add(classes_vX.Bullet(ocu.randrgb(),(Bob.rect.x+3 + width_player/2),bobo.rect.y,random.randint(-1*i+1,i-1),-15,[2,2]))
